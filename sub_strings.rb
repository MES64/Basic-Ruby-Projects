...
def substrings(string, dictionary)
  dictionary.each_with_object({}) do |word, sub_strings|
    times_found = string.downcase.scan(word.downcase).length
    sub_strings[word] = times_found if times_found.positive?
  end
end
...
puts "Correct: #{test2 == { 'down' => 1, 'go' => 1, 'going' => 1, 'how' => 2, 'howdy' => 1, 'it' => 2, 'i' => 3,
